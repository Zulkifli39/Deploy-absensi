...
const CheckOut = () => {
  ...
  const [mode, setMode] = useState("REGISTER"); // REGISTER atau CheckOut
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.post(API_PATHS.FACE.CHECK_OUT, formData, { headers: { "Content-Type": "multipart/form-data" } });
  ...
  return (
    <div className="p-4 mt-2 relative">
      ...
      {(loading || gettingLocation || checkingFace) && (
        <div className="fixed inset-0 z-70 bg-black/60 flex flex-col items-center justify-center">
          ...
          <p className="text-white font-semibold text-center">
            {checkingFace ? "Mengecek data wajah..." : gettingLocation ? "Mendeteksi lokasi..." : mode === "REGISTER" ? "Mendaftarkan wajah..." : "Check Out    ..."}
          </p>
        </div>
      )}
      ...
      <div className="flex items-center gap-28 mb-4">
        ...
        <span className="font-semibold text-lg">{mode === "REGISTER" ? "Daftar Wajah" : "Check Out"}</span>
      </div>
